...
```sh
/usr/bin/python3 bark_notify.py --setup-key
```

如果此前已经使用本项目的 Codex 版本，可直接安全迁移原有密钥，不会在终端输出密钥内容：

```sh
/usr/bin/python3 bark_notify.py --migrate-codex-key
```

发送测试通知：

```sh
/usr/bin/python3 bark_notify.py --test
```
...
```sh
/usr/bin/python3 bark_notify.py --setup-proxy http://127.0.0.1:7890
```

清除代理配置：

```sh
/usr/bin/python3 bark_notify.py --clear-proxy
```
